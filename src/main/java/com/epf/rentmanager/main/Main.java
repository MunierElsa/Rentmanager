...
public class Main {
	...
	public static void main(String[] args) {
		try {
			...
			Client nicolas = new Client(5, "Beque", "Nicolas", "nicolas.beque@epfedu.fr", date);

			/* Ajout du client */
			// System.out.println(ClientService.getInstance().create(nicolas));
			for (int i = 0; i < ClientService.getInstance().findAll().size(); i++) {
				System.out.println(ClientService.getInstance().findAll().get(i));
			}

			/* Suppression du client */
			// System.out.println(ClientService.getInstance().delete(nicolas));
			for (int i = 0; i < ClientService.getInstance().findAll().size(); i++) {
				System.out.println(ClientService.getInstance().findAll().get(i));
			}

			/* Création d'un véhicule */
			Vehicle vehicledeNicolas = new Vehicle(5, "Jaguar", (short) 18);
			...
			LocalDate debut = LocalDate.of(2000, 06, 25);
			LocalDate fin = LocalDate.of(2000, 07, 03); Reservation NicolasvaaNarbonne = new Reservation(1,1,1,debut,fin);
			  
			 // Ajout d'une reservation
			//System.out.println(VehicleService.getInstance().createResa(NicolasvaaNarbonne)); 
			for(int i = 0; i < VehicleService.getInstance().findAllResa().size();i++) { 
				System.out.println(VehicleService.getInstance().findAllResa().get(i));
			}
			  
			//Suppression d'une reservation
			//System.out.println(VehicleService.getInstance().deleteResa(NicolasvaaNarbonne)); 
			for(int i = 0; i < VehicleService.getInstance().findAllResa().size();i++) { 
				System.out.println(VehicleService.getInstance().findAllResa().get(i));
			}
